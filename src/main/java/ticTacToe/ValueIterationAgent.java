...
public class ValueIterationAgent extends Agent {
	...
	public void iterate()
	{
	    ...
	    for (int i = 0; i < k; i++) {
	        ...
	        // For each 'currState' in the key set of 'valueFunction'
	        for (Game currState : valueFunction.keySet()) {

	            // Check if the current state is not terminal
	            if (!currState.isTerminal()) {

	                // Value Initialization - 'nextVal' to negative infinity(lowest Value)
	                double nextVal = Double.NEGATIVE_INFINITY;

	                // For each possible 'move' in  'currState'
	                for (Move m : currState.getPossibleMoves()) {
	                	
	              

	                    // Value Initialization  'qVal' to 0
	                    double qVal = 0;

	                    // Value Iteration using Bellman's Equation
	                    for (TransitionProb t : mdp.generateTransitions(currState, m)) {
	                        qVal += t.prob * (t.outcome.localReward + discount * valueFunction.get(t.outcome.sPrime));
	                        

	                        //  Value Updated 'vNext' to be the maximum of 'nextVal' and 'qVal'
	                        // If nextValValue is unchanged ,convergence is likely
	                        nextVal = Math.max(nextVal, qVal);
	                     
	                    }

	                    // Value Updated of  the 'currState' to be 'nextVal'
	                    valueFunction.put(currState, nextVal);
	...
	{//Initialization of a new policy object 
		Policy p = new Policy();

		//Value iteration for each state in the 'valFunction' key set 
		for (Game currState : valueFunction.keySet()) {
			
			//Check if the current state is not terminal
			if (!currState.isTerminal()) {
				
				// Value Initialization - 'qValMax' to negative infinity(lowest Value)
				double qValMax=Double.NEGATIVE_INFINITY;
				//Value Initialization - 'optimalMove',the best moves set to 'null'
				Move optimalMove=null;
				
				//value iteration- Loop over each possible moves'm' in the 'currState'
				for (Move m:currState.getPossibleMoves()) {
					...
					//if 'qVal' is greater then maximum Q value then update 'qValMax' to 'qVal' and 'optimalMove' to current move 'm'
					if (qVal>qValMax) {
						qValMax=qVal;
						optimalMove=m;
				...
				//Update the policy 'p' of the 'currState' to 'optimalMove'
				p.policy.put(currState, optimalMove);
			}
		}
